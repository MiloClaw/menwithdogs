 ...
 * 
 * INVARIANT: This function only creates checkout sessions.
 * Subscription status is tracked via check-subscription, not here.
 */
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: "price_1Sw7MD3mECGw4pQtwaQVhRF8", // Pro Personalization $1.99/month
          quantity: 1,
        },
      ],
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/settings?tab=account&subscription=success`,
