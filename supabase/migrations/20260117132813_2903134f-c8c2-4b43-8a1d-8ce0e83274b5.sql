-- ═══════════════════════════════════════════════════════════════
-- PRO SIGNAL FIX: Phase 3
-- Update compute_user_affinity to include pro_selection signals
-- with legacy_key fallback and pro_context_definitions joins
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FROM (
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- LEGACY PAID TUNING SIGNALS (backward compatibility)
    -- ═══════════════════════════════════════════════════════════════
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- PRO SELECTION SIGNALS (NEW 4-STEP FLOW)
    -- CRITICAL: Only influence_mode = 'boost' options affect rankings
    -- Step 1 (about.*) has influence_mode = 'overlap' and is EXCLUDED
    -- Includes legacy_key fallback for backward compatibility
    -- ═══════════════════════════════════════════════════════════════
    
    -- PRO: Context density boost (primary path when density data exists)
    SELECT 
      p.primary_category as category,
      LEAST(pctx.default_confidence_cap, 
        pcd.density_score * 
        POWER(0.5, EXTRACT(EPOCH FROM (NOW() - us.created_at)) / _context_decay_seconds) *
        0.3
      ) as weighted_score
    FROM user_signals us
    JOIN pro_context_definitions pctx 
      ON (pctx.key = us.signal_key OR pctx.legacy_key = us.signal_key)
      AND pctx.is_active = true
      AND pctx.influence_mode = 'boost'
    JOIN place_context_density pcd 
      ON pcd.context_key = pctx.key 
      AND pcd.meets_k_threshold = true
    JOIN places p 
      ON p.id = pcd.place_id 
    ...
    -- PRO: Context priors fallback (scaffolding for new cities)
    SELECT 
      p.primary_category as category,
      LEAST(0.1, 
        pcp.confidence * 
    ...
    JOIN pro_context_definitions pctx 
      ...
      AND pctx.is_active = true
      AND pctx.influence_mode = 'boost'
    JOIN place_context_priors pcp 
      ON pcp.context_key = pctx.key
    JOIN places p 
      ON p.id = pcp.place_id 
      AND p.status = 'approved'
    LEFT JOIN place_context_density pcd 
      ON pcd.place_id = pcp.place_id 
      AND pcd.context_key = pctx.key 
      AND pcd.meets_k_threshold = true
    WHERE us.user_id = _user_id
      AND us.signal_type = 'pro_selection'
      AND us.signal_value = 'true'
