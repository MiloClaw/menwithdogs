-- Update compute_user_affinity to support pro_selection signals with influence_mode filter
-- This adds UNION blocks for the new 4-step Pro Settings flow
...
BEGIN
  ...
  FROM (
    ...
    -- PAID TUNING SIGNALS (existing - keep for backward compatibility)
    SELECT 
    ...
    -- PRO CONTEXT DENSITY SIGNALS (existing - keep for backward compatibility)
    SELECT 
    ...
    JOIN pro_context_definitions pctx 
      ON pctx.key = us.signal_key 
      AND pctx.is_active = true
      AND pctx.influence_mode = 'boost'  -- CRITICAL: Identity never ranks
    JOIN place_context_density pcd 
      ON pcd.context_key = us.signal_key 
    ...
    -- PRO SELECTION PRIORS (scaffolding for new cities)
    SELECT 
      p.primary_category as category,
      LEAST(0.1, 
        pcp.confidence * 
