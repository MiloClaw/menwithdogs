-- ═══════════════════════════════════════════════════════════════════════
-- PHASE 5: Deprecate legacy social columns (DRIFT-LOCK)
-- ═══════════════════════════════════════════════════════════════════════
-- These columns are NOT used in the intelligence layer and MUST NOT be used
-- Any new usage is a PR-blocking violation per product thesis
-- ═══════════════════════════════════════════════════════════════════════

COMMENT ON COLUMN public.couples.about_us IS 'DEPRECATED: Legacy profile field. Do not use. DRIFT-LOCK.';
COMMENT ON COLUMN public.couples.profile_photo_url IS 'DEPRECATED: Legacy profile field. Do not use. DRIFT-LOCK.';
COMMENT ON COLUMN public.couples.preferred_meetup_times IS 'DEPRECATED: Social scheduling field. Do not use. DRIFT-LOCK.';
COMMENT ON COLUMN public.member_profiles.social_settings IS 'DEPRECATED: Legacy visibility settings. Do not use. DRIFT-LOCK.';
COMMENT ON COLUMN public.events.social_energy_level IS 'DEPRECATED: Identity-adjacent field. Consider removal. DRIFT-LOCK.';

-- ═══════════════════════════════════════════════════════════════════════
-- PHASE 2 + PHASE 3: Update compute_user_affinity with blog intelligence and recency decay
-- ═══════════════════════════════════════════════════════════════════════
-- 
-- Changes:
-- 1. Add view_blog_post signals mapped via context_json->>'place_id' (weight 0.2)
-- 2. Apply 90-day half-life decay to behavioral signals:
--    - view_place, view_event, click_external, filter_category, view_blog_post
-- 3. NO decay applied to:
--    - couple_favorites joins (current state, not historical)
--    - explicit_preference (stable until changed)
--    - Paid tuning signals (already confidence-capped)
--    - save/unsave signals (handled via table state or negative weight)
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FROM (
    ...
    -- NEW: Blog views mapped to linked place category with decay (weight 0.2 base)
